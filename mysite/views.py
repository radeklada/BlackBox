...
def show_tests(request, id):
	...
	if request.user.is_authenticated:
		...
		if app_.owner == request.user:
			...
			return render(request, template_name='mysite/tests.html', context={'form': form, 'tests': tests, 'app_id': id, 'app_name':app_.name})
	...
	if request.user.is_authenticated:
		...
		if test_.app.owner == request.user:
			...
			if test_.app.owner == request.user:
				...
				return render(request, template_name='mysite/steps.html', context={'form': form, 'steps': steps, 'test_name': test_.name, 'app_name':test_.app.name})
